...
class _ContactsScreenState extends State<ContactsScreen> with SingleTickerProviderStateMixin {
  ...
  final _groupBox = Hive.box<ContactGroup>('contact_groups');
  List<Contact> contacts = [];
  List<ContactGroup> groups = [];
  int _selectedTab = 0; // 0 = Contacts, 1 = Groups
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildMenuItem(
                              ...
                              onTap: () {
                                _closeMenu();
                                Future.delayed(const Duration(milliseconds: 220), _showCreateGroupModal);
                              },
                              showDivider: true,
                            ),
  ...
  void _loadGroups() => setState(() {
        groups = _groupBox.values.toList()
          ..sort((a, b) => b.createdAt.compareTo(a.createdAt));
      });
              ...
              children: [
                ...
                Expanded(
                  child: _selectedTab == 0
                      ...
                      : (groups.isEmpty ? _buildEmptyGroupState() : _buildGroupList()),
                ),
                Padding(
            ...
            Expanded(
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  child: Text('Contacts', style: TextStyle(color: _selectedTab == 0 ? Colors.white : _Palette.mutedLight, fontSize: 13, fontWeight: _selectedTab == 0 ? FontWeight.w600 : FontWeight.w500)),
                ),
              ),
            ),
            Expanded(
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  alignment: Alignment.center,
                  child: Text('Groups', style: TextStyle(color: _selectedTab == 1 ? Colors.white : _Palette.mutedLight, fontSize: 13, fontWeight: _selectedTab == 1 ? FontWeight.w600 : FontWeight.w500)),
                ),
              ),
            ),
          ],
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.groups_rounded, color: Colors.white.withValues(alpha: 0.3), size: 28),
            ),
            const SizedBox(height: 16),
            const Text(
              'No groups yet',
            ...
            const SizedBox(height: 6),
            Text(
              'Tap ⋮ above to create your first group.',
              textAlign: TextAlign.center,
              style: TextStyle(color: _Palette.muted, fontSize: 13.5, height: 1.4),
            ),
          ],
  ...
  Widget _buildGroupList() {
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        ...
        return _GroupCard(
          group: g,
          onTap: () => _showEditGroupModal(g),
          onDelete: () => _showDeleteGroupConfirmation(g),
        );
      },
    );
  }

  void _showCreateGroupModal({ContactGroup? existingGroup}) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (ctx) => _CreateGroupModal(
  ...
  void _showEditGroupModal(ContactGroup group) {
    _showCreateGroupModal(existingGroup: group);
  }

  void _showDeleteGroupConfirmation(ContactGroup group) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: _Palette.surface,
        title: const Text('Delete Group?', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w600)),
        content: Text('Are you sure you want to delete "${group.name}"? Contacts within the group will not be deleted.', style: TextStyle(color: _Palette.muted, fontSize: 14)),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: Text('Cancel', style: TextStyle(color: _Palette.mutedLight))),
          TextButton(
            onPressed: () {
              group.delete();
              _loadGroups();
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      '${group.contactKeys.length} members',
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                      style: TextStyle(color: _Palette.muted, fontSize: 13, fontFamily: 'monospace'),
                    ),
                  ],
                ),
              ),
...
class _CreateGroupModal extends StatefulWidget {
  final ContactGroup? existingGroup;
  final VoidCallback onSave;

  const _CreateGroupModal({this.existingGroup, required this.onSave});
...
class _CreateGroupModalState extends State<_CreateGroupModal> {
  ...
  // Store as (hiveKey, Contact) pairs so we can retrieve the key without HiveObject.
  List<MapEntry<dynamic, Contact>> _all = [];
  List<MapEntry<dynamic, Contact>> _filtered = [];
  Set<String> _selectedKeys = {};

  @override
  void initState() {
    super.initState();
    final keys = _contactBox.keys.toList();
    final values = _contactBox.values.toList();
    final pairs = List.generate(keys.length, (i) => MapEntry(keys[i], values[i]))
    ...
    if (widget.existingGroup != null) {
      ...
      _selectedKeys = Set.from(widget.existingGroup!.contactKeys);
    }
  }
  ...
  void _save() async {
    ...
    if (widget.existingGroup != null) {
      widget.existingGroup!.name = name;
      widget.existingGroup!.contactKeys = _selectedKeys.toList();
      await widget.existingGroup!.save();
    } else {
      final g = ContactGroup(
        name: name,
        contactKeys: _selectedKeys.toList(),
      );
      await _groupBox.add(g);
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: EdgeInsets.only(bottom: bottom + 2, left: 1, right: 1),
      child: ClipRRect(
        borderRadius: const BorderRadius.vertical(top: Radius.circular(24)),
        child: BackdropFilter(
          filter: ImageFilter.blur(sigmaX: 30, sigmaY: 30),
          child: Container(
            ...
            child: Column(
              children: [
                ...
                Expanded(
                  child: _filtered.isEmpty
                      ? Center(child: Text('No contacts', style: TextStyle(color: _Palette.muted, fontSize: 15)))
                      : ListView.builder(
                                  ...
                                  children: [
                                    Container(
                                      ...
                                      child: Text(
                                        ...
                                        style: TextStyle(color: sel ? Colors.greenAccent : Colors.white, fontSize: 16, fontWeight: FontWeight.w700),
                                      ),
                                    ),
                                    const SizedBox(width: 14),
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          if (c.phones.isNotEmpty) ...[
                                            const SizedBox(height: 3),
                                            Text(c.phones[0], style: TextStyle(color: Colors.white.withValues(alpha: 0.5), fontSize: 13)),
                                          ]
                                        ],
                                      ),
                                    ),
